...
19. **`tiun.checkout()` authenticates on its own — never gate a purchase CTA behind login.** An anonymous visitor who clicks a plan signs up or logs in inside the checkout overlay and comes out entitled; `userChange` then fires with `event: 'login'` and `event: 'checkout'`. Map each CTA by what it *means*, not by whether anyone is signed in: a CTA naming a plan, price, or product ("Subscribe", "Get Pro", "Buy", a pricing-card button) → `tiun.checkout({ productId })`. A CTA about identity only ("Log in", "Sign in", "My account") → `tiun.login()`. Never chain the two (`login()` then `checkout()`), never wrap checkout in an `isAuthenticated` check, and never insert a login step in front of a plan. When you are generating the UI yourself, produce one checkout CTA per product plus **one separate** login link for returning customers — a parallel entry point, not a prerequisite. See `references/subscriptions.md`.
...
- User has a pricing page, plan cards, or "Subscribe" / "Buy" buttons → map each one straight to `tiun.checkout({ productId })`. Only a bare "Log in" / "Sign in" / account CTA maps to `tiun.login()` (Rule 19).
