...
- **Gating the buy button behind login.** `tiun.checkout()` handles sign-up and login inside the purchase flow, so the buy CTA maps straight to it. Login belongs *beside* it for returning customers, never in front of it. See [subscriptions.md](subscriptions.md#wiring-ctas-to-sdk-methods).
