...
- **Gating the buy button behind login.** `tiun.checkout()` handles sign-up and login inside the purchase flow, so the buy CTA maps straight to it. Login belongs *beside* it for returning customers, never in front of it. See [subscriptions.md](subscriptions.md#wiring-ctas--checkout-vs-login).
- **Reaching into the checkout overlay.** It is shadow DOM and off limits — no injected copy, no CSS overrides. Put "you already own this" style messaging on your own page. See [overview.md](overview.md#hosted-ui-is-a-black-box).
